...
class WinRM(cskcommands.cmd.CSKCommand):
    ...
    API = "winrm_command"
    Examples = '''
    ccl-winrm -uusername -ppassword -sserver -c"dir c:"
'''
...
class WinRMPS(cskcommands.cmd.CSKCommand):

    Description = 'Test WinRM connections and issue commands.'
    API = "winrm_command"
    Examples = '''
    ccl-powershell -uusername -ppassword -sserver -c"write-host 'Hello World'"
'''
    Options = [Param(name='server', short_name='s', long_name='server',
                     optional=True, ptype='string',
                     doc='Windows server address.'),
